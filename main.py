...
from modify import on_tree_select
...
class Modify:
    def __init__(self, table_name, schema_name, modify_notebook):
        ...
        self.tree.bind("<<TreeviewSelect>>", on_tree_select(self))
...
# ---------------------- Search Tab ---------------------- #
    ...
    search_frame.pack(padx=8, pady=8, fill='both', expand=True)
    ...
    tk.Button(button_frame, text="Serach", command=lambda: search_record(), width=12).grid(row=0, column=0, padx=4)
    ...
    Search_tree = ttk.Treeview(search_tab)
    Search_tree.pack(fill="both", expand=True, padx=16, pady=8)
